...
bool showInfo(List *plist)
{
    ...
    case '2':
        ...
        for (i = 0; i < plist->items; ++i)
            if (!strcmp(plist->entries[i].name, temp))
            {
                NodePrint(plist->entries[i]);
                if (!status)
                    status = true;
            }
        break;
    case '3':
        ...
        scanf_s("%s", temp, 15);
        for (i = 0; i < plist->items; ++i)
            if (!strcmp(plist->entries[i].lesson, temp))
            {
                NodePrint(plist->entries[i]);
                if (!status)
                    status = true;
    ...
    case '4':
        ...
        scanf_s("%s", temp2, 15);
        for (i = 0; i < plist->items; ++i)
            if (!strcmp(plist->entries[i].name, temp) && !strcmp(plist->entries[i].lesson, temp2))
            {
                NodePrint(plist->entries[i]);
                if (!status)
                    status = true;
